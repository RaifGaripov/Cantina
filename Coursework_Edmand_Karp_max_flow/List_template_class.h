...
public:
	List_template_class() : head(NULL), tail(NULL), size(0) {}
	List_template_class(const List_template_class<T>& list) {
		head = NULL;
		tail = NULL;
		Node* temp = list.head;
		while (temp) {
	...
	void push_back(List_template_class<bool> ls2)
	{
		Node* temp = ls2.head;
	...
	void push_front(List_template_class& ls2)
	{
		Node* temp = ls2.tail;
		size_t n = ls2.get_size();
		size += ls2.get_size();
